...
( function( jQuery ) {
	...
	( function() {
		...
		menuToggle.on( 'click.revert', function() {
	...
	( function() {
		...
		function toggleFocusClassTouchScreen() {
			if ( window.innerWidth >= 910 ) {
				jQuery( document.body ).on( 'touchstart.revert', function( e ) {
					if ( ! jQuery( e.target ).closest( '.main-navigation li' ).length ) {
						jQuery( '.main-navigation li' ).removeClass( 'focus' );
					}
				} );
				siteNavigation.find( '.menu-item-has-children > a' ).on( 'touchstart.revert', function( e ) {
				...
				siteNavigation.find( '.menu-item-has-children > a' ).unbind( 'touchstart.revert' );
			}
		}

		if ( 'ontouchstart' in window ) {
			jQuery( window ).on( 'resize.revert', toggleFocusClassTouchScreen );
			toggleFocusClassTouchScreen();
		}

		siteNavigation.find( 'a' ).on( 'focus.revert blur.revert', function() {
	...
	jQuery( document ).ready( function() {
		...
		jQuery( window )
			.on( 'load.revert', onResizeARIA )
			.on( 'resize.revert', function() {
